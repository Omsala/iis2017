...
smile = smile_cascade.detectMultiScale(bw,1.2,1)  
eyes = eye_cascade.detectMultiScale(bw,1.2,1)
...
def getTarget():
        while True:
           ret, frame = cap.read()
           bw2 = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
           face2 = face_cascade.detectMultiScale(bw2, 1.3, 5)
           for (x,y,w,h) in face:
